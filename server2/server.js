...
// --- List All Users ---
// (must be after app is initialized)
...
app.post('/api/game/join', async (req, res) => {
  ...
  if (game.players.length >= 5) return res.status(400).json({ error: 'Game full' });
  // Assign next available role
  const nextRole = `player${game.players.length + 1}`;
  await games.updateOne({ gameId }, {
    $push: { players: username },
    $set: { [`roles.${username}`]: nextRole, status: 'active' }
  });
  const updated = await games.findOne({ gameId });
  res.json({ message: 'Joined game', game: updated, role: nextRole });
});
